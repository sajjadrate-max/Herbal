...
${questionList}`;
    ...
    let relevantIds = Array.isArray(parsed.relevantIds) ? parsed.relevantIds : [];
    const matchedDiseaseNames = Array.isArray(parsed.matchedDiseaseNames) ? parsed.matchedDiseaseNames : [];

    // حفاظتی جانچ: اگر تکلیف کی تحریر میں عمر پہلے ہی صاف لکھی ہو تو "عمر کتنی ہے" سوال دوبارہ نہ دکھائیں،
    // چاہے AI نے غلطی سے اسے شامل کر دیا ہو۔
    const ageAlreadyStated = /\d[\d\s,،\-٫]*\s*(سال|برس|ماہ|مہینے)/.test(complaint);
    if (ageAlreadyStated) {
      relevantIds = relevantIds.filter(id => id !== 'age');
    }

    // حفاظتی جانچ: درد سے متعلق سوالات صرف اسی وقت رہنے دیں جب تحریر میں واقعی درد کا ذکر ہو،
    // چاہے AI نے موضوع سے غیر متعلق طور پر شامل کر دیا ہو۔
    const painMentioned = /درد|تکلیف دہ|دکھ(تا|تی|نے)/.test(complaint);
    if (!painMentioned) {
      relevantIds = relevantIds.filter(id => id !== 'joint_time' && id !== 'joint_position');
    }
